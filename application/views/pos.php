          ...
          <div class="box box-primary">
            ...
            <?php } ?>
              ...
              <div class="row">
                <div class="col-md-6">
                   <div class="input-group">
                     <span class="input-group-addon" title="Sales Mode"><i class="fa fa-exchange"></i></span>
                     <select class="form-control" id="sales_mode" name="sales_mode" style="width: 100%;">
...
function addrow(id='',item_obj=''){
    ...
    var sales_price;
    if(sales_mode == 'Wholesale'){
         sales_price = (item_obj=='') ? $('#div_'+id).attr('data-item-wholesale-price') : item_obj.wholesale_price;
    } else {
         sales_price = (item_obj=='') ? $('#div_'+id).attr('data-item-sales-price') : item_obj.sales_price ; 
    } 
...
function get_item_details(item_id){
  ...
  $.ajax({
    type: "POST",
    url: "<?php echo $base_url; ?>pos/get_item_details",
    ...
    success: function(result){
      ...
      obj['wholesale_price']= item['wholesale_price'];
...
//BARCODE SCANNER: ON ENTER KEY
$("#item_search").on("keypress",function(e){
    if(e.which == 13){
        e.preventDefault();
        var code = $(this).val();
        if(code != ''){
            check_item_by_code(code);
        }
        return false;
    }
});

function check_item_by_code(code){
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
    $.ajax({
      type: "GET",
      ...
      success: function(res){
          ...
          if (res.length) {
              ...
              $.each(res, function(i, el){
                  if(el.item_code === code || el.custom_barcode === code || el.value === code){
                      get_item_details(el.id);
                      $("#item_search").val('');
                      found = true;
                      return false;//break
                  }
              });
              //If not exact match, but only 1 result, take it
              if(!found && res.length == 1){
                   get_item_details(res[0].id);
                   $("#item_search").val('');
                   found = true;
              }
          }
          ...
          if(!found){
              toastr["error"]("Item Not Found or Multiple Matches!");
              $("#item_search").autocomplete("search"); //Forward to autocomplete
          } else {
